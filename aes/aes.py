...
def generateKeySchedule(cipherkey, keysize):
    expandedkey = []
    size = 0
    iteration = 1
    tempbytes = [0x00, 0x00, 0x00, 0x00]
    ...
    while size < 240:
        for i in range(4):
            tempbytes[i] = expandedkey[(size-4)+i]
        if size % keysize == 0:
            tempbytes = schedule(tempbytes, iteration)
            iteration += 1
...
if __name__ == '__main__':
    ...
    test_plaintext = [0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]
    test_cipherkey = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f]
