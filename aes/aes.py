...
def shiftRows_inv(state_array):
    ...
    for col in range(4):
        for row in range(4):
            state_array[col][row] = state_orig[(4 - row + col) % 4][row]
    return state_array
...
def state_to_hexstring(state):
    ...
    for arr in state:
        for num in arr:
            if len(hex(num)) == 3:
                s.append(hex(num)[:2] + '0' + hex(num)[-1])
            else:
                s.append(hex(num))
    # return ''.join([letters[2:].upper() for letters in s])
...
def encrypt(state_array, key, num_rounds):
    ...
    print("addRoundKey round 0\t", state_to_hexstring(state_array))
    for index in range(1, num_rounds):
        round_key = genRoundKey(expandedKey, index)
        state_array = subBytes(state_array)
        print("subBytes round {}\t".format(index),
              state_to_hexstring(state_array))
        state_array = shiftRows(state_array)
        print("shiftRows round {}\t".format(index),
              state_to_hexstring(state_array))
        state_array = mixColumns(state_array)
        print("mixColumns round {}\t".format(index),
              state_to_hexstring(state_array))
        state_array = addRoundKey(state_array, round_key)
        print("addRoundKey round {}\t".format(index),
              state_to_hexstring(state_array))
        print('\n')
...
def decrypt(state_array, key, num_rounds):
    ...
    round_key = genRoundKey(expandedKey, num_rounds)
    state_array = addRoundKey(state_array, round_key)
    print("post addRoundKey round {}\t".format(
        num_rounds), state_to_hexstring(state_array))
    state_array = shiftRows_inv(state_array)
    print("post shiftRows_inv round {}\t".format(
        num_rounds), state_to_hexstring(state_array))
    state_array = subBytes_inv(state_array)
    print("post subBytes round {}\t".format(
        num_rounds), state_to_hexstring(state_array))
    for index in range(num_rounds - 1, 0, -1):
        round_key = genRoundKey(expandedKey, index)
        state_array = addRoundKey(state_array, round_key)
        print("post addRoundKey round {}\t".format(
            index), state_to_hexstring(state_array))
        state_array = mixColumns_inv(state_array)
        print("post mixColumns_inv round {}\t".format(
            index), state_to_hexstring(state_array))
        state_array = shiftRows_inv(state_array)
        print("post shiftRows_inv round {}\t".format(
            index), state_to_hexstring(state_array))
        state_array = subBytes_inv(state_array)
        print("post subBytes_inv round {}\t".format(
            index), state_to_hexstring(state_array))
        print('\n')
    round_key = genRoundKey(expandedKey, 0)
    state_array = addRoundKey(state_array, round_key)
...
def get_file_and_pad(inputfile, mode):
    with open(inputfile, 'r') as fhandle:
        ascii_chars = fhandle.read()

        hex_chars = [ascii_chars[i: i + 2]
                     for i in range(0, len(ascii_chars), 2)]
        try:
            hex_chars.remove('\n')
        except ValueError:
            pass
...
def process_args_and_run(args):
    blocks = []

    input_byte_whole = get_file_and_pad(args.inputfile, args.mode)
        ...
        with open(args.outputfile, 'w') as whandle:
            for block_no, state in enumerate(blocks):
                ...
                if args.mode == 'encrypt':
                    ...
                    state = decrypt(state, key, num_rounds)
                print('\nFinal state for block {}\t'.format(
                    block_no), state_to_hexstring(state))
                whandle.write(state_to_hexstring(state))
    else:
        import sys
        sys.exit('Invalid mode. Choose "encrypt" or "decrypt"')
